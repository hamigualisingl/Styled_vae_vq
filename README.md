...
- 生成:数据不对齐;自回归下一个token预测概率分布比较平均;通俗来说,图像是二维的,一维度顺序生成图片token时候,token间紧密性不高,约束有限,如果可以一口气生成四周相邻的token,那么预测的自由度就会降低很多,预测就容易些.
- 解决方案：Styled_vae_vq,将图像按序离散成36个从高级到低级的属性token,力求编码器提供了合理的易对齐的相互牵扯的序列和用于初始化llm_added_token的词表,而非训出有强大拟和能力的解码器.使用时候可以和其他vqvae拼接.
