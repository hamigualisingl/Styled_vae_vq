...
- 生成:Vq-vae(内容的投影,语义弱,不好学习,类似于把man分词成m,a,n的感觉):自回归按展开顺序预测下一个token,缺乏整体布局观和纠错机制(画错一俩错能纠正回来),而且token语义弱,隐状态对应太多输出,预测难度较大.不论是理解还是生成,哪有像素级别生成的(vq-vae可以理解成像素级别表征),都是素材级别,
