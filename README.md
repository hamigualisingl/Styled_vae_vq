...
- 最新进展:第二阶段离散时候出现问题,分析如下:离散后特征出现误差.直接回归像素,缓和余地太小,解决方向-添加容错机制:比如考虑借用VQGan的成果,交叉熵方式预测它encoder出的序列结果,使用时候只取top1,然后借用它的解码器,考虑去掉占位token,避免前面token在重建时候,比重过大.不过之前的版本依然会在理解与生成测试,因为是对图像总结,提供尽可能一对一映射关系,会有细节编码器去生图,不要求自身生成效果.
