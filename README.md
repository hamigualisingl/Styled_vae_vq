...
- 最新进展:第二阶段离散时候出现问题,分析如下:离散后特征出现误差.解决方向-添加容错机制:借用VQGan的成果,交叉熵方式预测它encoder出的序列结果,去掉noise使用余弦距离,encoder结果与一个参数矩阵做相似度,然后归一化(操控温控系数),取加权后的向量送入解码器,这样即使离散后不能复原也能保证值连续,属性也是连续的.去掉占位token,避免前面token在重建时候,比重过大.不过之前的版本依然会在理解与生成测试,因为是对图像总结,提供尽可能一对一映射关系,会有细节编码器去生图,不要求自身生成效果.
- 最新进展:详情见modelv2.py
