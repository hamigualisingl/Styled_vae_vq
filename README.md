...
- 最新进展:第二阶段离散时候出现问题,分析如下:离散后特征出现误差.直接回归像素,缓和余地太小,考虑借用MaskGIT-VQ的成果,交叉熵方式预测它的表征,然后借用它的解码器,考虑去掉占位token,避免前面token在重建时候,比重过大.不过之前的版本依然会在理解与生成测试,因为我对图像总结,提供尽可能一对一映射关系,会有细节编码器去生图.
