...
- 理解:目前视觉encoder输出token语义弱,规则弱(vqgan也是如此,给生成带来挺大困难),llm注意力重点偏移,出现幻觉;token数量多,多图和视频理解时候增加计算负担(目前减少token的做法都是间接还原图像,bad case很多,图像信息损失多);通俗来说,说话没有重点,没有规则逻辑.(经过实验:titok也未观察出规律)
