...
- 生成:数据不对齐(输入输出为一对n的映射关系),加上按照一维顺序预测下一个token,token联系不够紧密.下一个token预测,隐状态对应太多输出,概率分布比较平均,预测难度较大.(经实验普通的vqvae自回归生图方案,前面给足原图的多尺度小图token,是很容易生成原图的token的,但是直接生成小图token,存在多样性问题)
