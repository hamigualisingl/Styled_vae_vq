...
## 动机： 
应对图像理解与自回归生成难题,比如:理解-注意力分散,出现幻觉;生成-数据不对齐,下一个token概率分布比较平均,自回归生成困难,提出Styled_vae_vq,将图像按序离散成36个从高级到低级的属性token,力求编码器提供了合理的易对齐的序列和用于初始化llm_added_token的词表,而非训出有强大拟和能力的解码器.使用时候可以和其他vqvae拼接.
...
- 阶段二:通过k-means聚类,得到词表(n,128).img->编码器(固定)输出连续值->量化(固定)->升维度(参与训练,维度为llm词表维度一致,用于llm的词表初始化)->降维度(参与训练)->解码器(固定).
...
- ### Pretrained Model Weight
    ...
    数据集:主要是在cc3m随机抽选的56万张图片训练,然后在imagenet等其他数据进一步训练.很快放出来,目前第一阶段(56万张图片)已经训练完成,训练数据外的图片重建效果不错(泛化能力)(目前模型大小为8.9G,主要是编码器较大),后面有机会会在laion400m进一步训练.
