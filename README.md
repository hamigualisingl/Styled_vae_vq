...
## 动机： 
应对图像理解与自回归生成难题,比如:理解-token数量多,地位平等,注意力分散,出现幻觉;生成-数据不对齐,下一个token概率分布比较平均,自回归生成困难,提出Styled_vae_vq,将图像按序离散成36个从高级到低级的属性token,力求编码器提供了合理的易对齐的序列和用于初始化llm_added_token的词表,而非训出有强大拟和能力的解码器.使用时候可以和其他vqvae拼接.
